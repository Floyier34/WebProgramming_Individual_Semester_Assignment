...
Based on code review, here are recommended improvements:

1. **Security**
   - [ ] Implement login attempt throttling and rate limiting
   - [ ] Add CSRF token protection for all forms
   - [ ] Implement session timeout after 30 minutes inactivity
   - [ ] Validate brand_id and category_id exist before INSERT/UPDATE
...
3. **Code Quality**
   - [ ] Unify database connections (use PDO everywhere)
   - [ ] Implement error logging instead of exposing errors
...
4. **Performance**
   ...
   - [ ] Add pagination for admin product lists

5. **User Experience**
   - [ ] Add product image gallery
   - [ ] Implement real-time cart updates
   - [ ] Add product filters/faceted search
...
2. Examine `config/db_conn.php` to understand database connection
3. Look at `public/index.php` to see how data flows from DB to frontend
4. Review `php/action-add-device.php` to understand CRUD operations
5. Study `php/auth.php` to understand authentication

### Making a Simple Change
Example: Adding a new field to products
1. Modify database table: `ALTER TABLE devices ADD COLUMN new_field VARCHAR(255);`
2. Update form: `admin/add-device.php` - add form input
3. Update action: `php/action-add-device.php` - handle new field
4. Update display: `public/device.php` and `php/partials/device-card.php` - show field
5. Update query: `php/func-device.php` if needed

### Debugging Tips
- Check browser console for JavaScript errors
- View page source to see generated HTML
- Use var_dump() to inspect PHP variables
- Check server error logs in XAMPP
- Add temporary echo statements to trace execution flow
